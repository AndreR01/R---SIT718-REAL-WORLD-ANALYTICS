...
I <- c(chlorides) # Choose any four X variables and Y

wines.sample <- wines[sample(1:500, 400), c(1,3,4,5:6)]
wines.sample
...
#Use log transformation for chlorides due to strong postive skew
min_log <-log(min(wines.sample[,2]))
max_log <-log(max(wines.sample[,2]))
log_chlorides <- (1-((log(wines.sample[,2])-min_log)/(max_log-min_log)))
log_chlorides
...
#Use square root transformation for sulfur dioxide as not as skewed as chlorides
min_sqrt_sulfur_dioxide <-sqrt(min(wines.sample[,3]))
max_sqrt_sulfur_dioxide <-sqrt(max(wines.sample[,3]))
sqrt_sulfur_dioxide <- (1-((sqrt(wines.sample[,3])-min_sqrt_sulfur_dioxide)/(max_sqrt_sulfur_dioxide-min_sqrt_sulfur_dioxide)))
sqrt_sulfur_dioxide
...
#Normalise alcohol with log transformation and standardise min/max
